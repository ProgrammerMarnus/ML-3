...
class PerformanceMetrics:
    ...
    def calculate_alpha_beta(
        ...
        alpha = (
            PerformanceMetrics.calculate_annualized_return(excess_strat, periods_per_year)
            - beta * PerformanceMetrics.calculate_annualized_return(excess_bench, periods_per_year)
        )
...
class MetricsCollector:
    ...
    def get_prediction_accuracy(self, days: int = 30) -> Dict[str, float]:
        ...
        return {
            ...
            "mape": np.mean([abs(e / p["actual"]) for e, p in zip(errors, recent_preds) if p["actual"] != 0]) * 100,
